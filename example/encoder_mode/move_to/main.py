...
md20_obj = md20.Md20(i2c, i2c_address=md20.DEFAULT_I2C_ADDRESS)
...
speed = 60
while True:
    if trigger_time == 0 or time.ticks_ms() - trigger_time > 4000:
        trigger_time = time.ticks_ms()
        target_position = 720 if target_position == 0 else 0
        for i in range(md20.MOTOR_NUM):
            print(f"motor {i} move to {target_position}")
            md20_obj[i].move_to(target_position, speed)
